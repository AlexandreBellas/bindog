...
export default function LocaleSwitcher() {
    ...
    return (
        <div className="flex items-center gap-1.5" aria-label={m.language_label()}>
            <Select value={currentLocale} onValueChange={value => setLocale(value as ILocale)}>
                <SelectTrigger
                    id="locale-switcher"
                    aria-label={m.language_label()}
                    className="h-9 max-w-44 cursor-pointer rounded-full border border-(--chip-line) bg-(--chip-bg) px-3 text-xs font-semibold text-(--bark) shadow-[0_8px_22px_rgba(90,55,25,0.08)] sm:max-w-none sm:text-sm"
                >
                    <SelectValue />
                </SelectTrigger>
